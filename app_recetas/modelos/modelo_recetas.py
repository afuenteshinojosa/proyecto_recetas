...
class Receta:
    ...
    def validar_formulario_recetas (data):
        ...
        if len(data ['nombre']) < 3:
            es_valido = False
            flash("Tienes que ingresar el nombre de la receta.", "error_nombre")
        if len(data ['descripcion']) < 3:
            es_valido = False
            flash("Tienes que ingresar la descripcion de la receta.", "error_descripcion")
        if len(data ['instrucciones']) < 3:
            es_valido = False
            flash("Tienes que ingresar las instrucciones de la receta.", "error_instrucciones")
        if data ['fecha_elaboracion'] == "":
            es_valido = False
            flash("Tienes que ingresar la fecha de elaboracion de la receta.", "error_fecha_elaboracion")
        if "menos_treinta" not in data:
            es_valido = False
            flash("Informa si la receta se elabora en menos de 30 min.", "error_menos_treinta")
        return es_valido
